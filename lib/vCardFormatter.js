		...
		getFormattedString: function(vCard) {
			...
			if (vCard.otherEmail) {
				...
				vCard.otherEmail.forEach(
					function(address) {
						if (majorVersion >= 4) {
							formattedVCardString += 'EMAIL' + encodingPrefix + ';type=' + address.title + ':' + e(address.value) + nl();
						} else if (majorVersion >= 3 && majorVersion < 4) {
							formattedVCardString += 'EMAIL' + encodingPrefix + ';type=' + address.title + ':' + e(address.value) + nl();
						} else {
							formattedVCardString += 'EMAIL' + encodingPrefix + ';OTHER;INTERNET:' + e(address.value) + nl();
						}
					}
				);
			}
			...
			if (vCard.otherPhone) {
				...
				vCard.otherPhone.forEach(
					function(number) {
						if (majorVersion >= 4) {
							formattedVCardString += 'TEL;VALUE=uri;TYPE=' + number.title + ':' + e(number.value) + nl();
						} else {
							formattedVCardString += 'TEL;TYPE='  + number.title + ':' + e(number.value) + nl();
						}
					}
				);
			}
